...
\subsection{信息、动作与证据链}
题目中的一次“测量”同时改变三类状态：机器狗位置、频道的可能区域以及虚拟时钟。为了避免把接口语义和隐藏真值混在一起，本文把运行记录拆成以下四类事件。每类事件都对应一个可回放字段，后续的图表只从这些字段计算，不在程序外手工改写。表~\ref{tab:q1-evidence} 给出了事件、状态更新和证据字段的对应关系。

\begin{table}[H]\centering\small
\caption{接口反馈到论文证据的映射}\label{tab:q1-evidence}
\begin{tabular}{p{0.19\linewidth}p{0.34\linewidth}p{0.33\linewidth}}
\toprule
事件 & 在线状态更新 & 可核验输出 \\
\midrule
\texttt{direction} & 将带误差的楔形加入对应频道的 $\mathcal P_k$ & 示向度、测量点、更新后的 MEC \\
\texttt{near} & 保留位置并允许同点清除 & 清除请求、成功标识、动作时间 \\
\texttt{no\_signal} & 记录一次排查，不删除频道 & 频道、站点、响应和累计次数 \\
网络/协议失败 & 不推进位置和虚拟时钟，使用同一请求标识重试 & 请求 ID、重试次数、最终响应 \\
\bottomrule
\end{tabular}
\source{题面接口语义与 src/cumcm\_b/protocol.py 的日志字段。}
\end{table}

因此，论文中的“发现”只表示得到过有效方向或近距离反馈，“清除”只表示接口返回成功，“完成”还要求所有覆盖站访问结束并满足完成证书。三者不是同一个布尔量。这个区分尤其影响问题四：定向源在某个站点返回 \texttt{no\_signal} 只说明当前测量没有接收，不足以证明该频道为空；只有覆盖证书和后续定位证书共同成立，程序才允许退出。
...
\node[flowbox,fill=paleBlue] (a) {题面/接口反馈};
\node[flowbox,fill=paleTeal,right=of a] (b) {集合状态更新};
\node[flowbox,fill=paleSand,right=of b] (c) {下一停靠点};
\node[flowbox,fill=mist,right=of c] (d) {清除证书};
\draw[flowline] (a)--(b);\draw[flowline] (b)--(c);\draw[flowline] (c)--(d);
\draw[flowline,rounded corners] (d.south) |- ++(0,-12mm) -| (a.south);
\end{tikzpicture}
