...
\paragraph{外包性质。}每个 $W_i$ 是两个闭半平面的交，因而是闭凸集；有限个 $W_i$ 与接收圆盘外切多边形的交仍为闭凸集。由于真实源位置同时满足题面的误差界和接收条件，必有 $g_{\rm true}\in\mathcal P\subseteq\mathcal P_{\rm out}$。所以程序宁可保留外切多边形带来的少量冗余，也不在顶点离散时删除真实位置。这个包含关系是后续清除证书的逻辑起点：对外包区域成立的半径上界，才可以推出真实源点一定可清除。
...
为避免几何求解器的单点故障，本文在三个位置做独立核验：第一，用顶点对穷举复核旋转卡壳的直径；第二，用全部顶点到 MEC 圆心的最大距离复核包围半径；第三，在每次清除前重新读取当前外包，而不沿用上一次测量后的缓存值。若三者任一不一致，运行记录标记为不可清除并转入下一次测量。这样，数值容差只会使算法更保守，不会把误差方向偏向“更容易清除”。
...
\draw[thick,draw=ink] (A)--(B)--(C)--cycle;\draw[dashed,draw=coral] (19.5,11.258) circle (22.517);
\fill[navy] (19.5,11.258) circle (1.1);\draw[<->,draw=navy] (A)--(B) node[midway,below,text=ink] {$39\,\mathrm m$};
\node[text=ink] at (19.5,-9) {直径 $39$ 米，但包围圆半径 $22.52$ 米};
\end{tikzpicture}
...
\draw[draw=linegray] (-3,-2.2) rectangle (3.2,2.4);\fill[paleBlue] (-2.5,-1.5)--(2.4,-1.5)--(1.0,1.8)--cycle;\draw[thick,draw=navy] (-2.5,-1.5)--(2.4,-1.5)--(1.0,1.8)--cycle;
\draw[dashed,draw=coral] (0.0,0.1) circle (1.35);\fill[navy] (0,0.1) circle (1.5pt);
\node[text=ink] at (0,-2.7) {楔形交集 $\mathcal P$ 与最小包围圆};
\end{tikzpicture}
