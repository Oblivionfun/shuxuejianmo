...
这里的“至少一个可接收站点”与“每个站点都能接收”是两个不同命题。对任意覆盖三角形，若三个顶点都位于发射半平面的反侧，则包含源点的凸组合会给出矛盾；因此只需保证三角形的三个顶点都在接收圆盘内，就能对未知朝向得到至少一个合法测量点。该证书不要求知道发射方向，也不把一次 \texttt{no\_signal} 解释成频道不存在。它把问题四的未知方向从在线决策中的不可见变量，转化成覆盖设计阶段可以检查的几何条件。
...
\source{ring25\_cover 的确定性构造；32 个覆盖单元，图中编号为访问顺序。}
\end{figure}
...
联合策略的在线步骤可以压缩为四步：
\begin{enumerate}
\item 按固定环形路线到达下一个覆盖站，顺序排查尚未清除的频道；
\item 对首次返回方向的频道建立 $\mathcal P_k$，对已有方向频道只保留当前半径最大的三个补充测向名额；
\item 覆盖完成后，以各频道 MEC 圆心为目标点，按开放路径距离做最近邻初始化和 2-opt 改善；
\item 每到一个末端目标先复算 MEC，证书成立则清除，否则继续滚动测向；超过测向预算才调用有限方格兜底。
\end{enumerate}
其中第二步是把“信息收益”限制在固定预算内，第三步才承担主要的路程压缩。若某一频道的可能区域很小，算法不会为了再次测量而偏离主路线；若区域仍大，则保留其风险状态并把兜底动作写入日志。该优先级使时间优化可以被拆解为覆盖站路程、目标巡回路程、主动测量和兜底四个可审计分量。

图~\ref{fig:q4-route} 是两阶段策略的抽象流程，图~\ref{fig:q4-trajectory} 则把同一思想映射到过程日志中的实际动作和虚拟时间。

\begin{figure}[H]\centering
\begin{tikzpicture}[x=1cm,y=1cm]
\draw[thick,draw=ink,rounded corners] (-3,-1.9) rectangle (3.2,2.0);\draw[step=0.9,linegray!45] (-2.8,-1.7) grid (3,1.8);
\draw[very thick,navy,->] (-2.4,-1.3)--(-1.2,0.8)--(0.0,-1.2)--(1.1,1.3)--(2.6,-0.8);
\foreach \p in {(-2.4,-1.3),(-1.2,0.8),(0,-1.2),(1.1,1.3),(2.6,-0.8)}{\fill[coral] \p circle (2pt);}
\node[text=ink] at (0,-2.35) {覆盖阶段收集信息，末端阶段清除目标};
\end{tikzpicture}
...
\source{\nolinkurl{results/synthetic/q4_current_trace.json}；q4\_joint，ring25，种子 20260911，本地合成过程记录。}
\end{figure}
