...
| 问题 4 覆盖改进 | 49 站 → 25 站环形覆盖；50 个配对合成场景中相对 26 站联合策略再降低 **15.59%** |
| 正式测试与论文 | 尚无正式成绩；Overleaf 已同步一版可编译论文草稿，正式结果仍待替换 |

早期 20 局方格对比曾得到 17.11%；当前 50 局共同源配置中，25 站环形 `q4_joint` 比 26 站联合策略再降低 15.59%。两者都是本地合成结果；官方演练批次使用不同隐藏场景，客户端未通过 API 验证演练模式，也不能据此推算官方提升比例或奖项水平。[统计口径与限制](docs/results.md)

<table>
<tr>
<td width="50%"><img src="reports/figures/process_q4_coverage.png" alt="问题4三角网格覆盖构造"></td>
<td width="50%"><img src="reports/figures/result_q4_ring25.png" alt="问题4环形覆盖方案配对实验"></td>
</tr>
...
当前 q3 入口使用 `adaptive_q10_dynamic`：首次主动点优先控制移动，后续测量再追求 q10 信息增益；该选择来自 50 个共同源配置的本地配对实验。当前 q4 入口使用 `q4_joint` + `ring25`：25 站闭集环形覆盖、预先计算的固定开路径、覆盖阶段补充测向，覆盖结束后集中清除已发现目标。选择依据见 [q3 优化记录](docs/optimization.md) 和 [q4 优化记录](docs/q4_optimization.md)。
...
| 4 · 定向源 | 25 站环形三角覆盖、联合巡回、光学格子兜底 | 997 m 单元覆盖任意闭半平面方向 |
