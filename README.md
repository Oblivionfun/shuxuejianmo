...
17.11% 是相对本项目方格基线的本地合成结果；官方演练批次使用不同隐藏场景，客户端未通过 API 验证演练模式，也不能据此推算官方提升比例或奖项水平。[统计口径与限制](docs/results.md)
...
当前 q3 入口的最新稳定策略是 `adaptive_q25`：候选点使用 25% 分位误差并加入移动惩罚，最终仍以有界误差半平面交和最小包围圆证书清除。完整策略组演练中它的合并单源时间最低；`adaptive_q10_dynamic` 虽然总虚拟时间最低，但清除数较少，保留为显式研究对照。选择依据见 [q3 优化记录](docs/optimization.md)。
